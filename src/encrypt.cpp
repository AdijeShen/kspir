...
void encrypt(uint64_t *b, uint64_t *a, Secret &secret, uint64_t *message) {
  ...
  intel::hexl::EltwiseMultMod(b, a, secret.getData().data(), length, modulus,
                              1);
...
void encrypt(uint64_t *b1, uint64_t *a1, uint64_t *b2, uint64_t *a2,
  ...
  intel::hexl::EltwiseMultMod(b1, a1, secret1, length, modulus1, 1);
  intel::hexl::EltwiseMultMod(b2, a2, secret2, length, modulus2, 1);

  std::vector<uint64_t> err(length);
  sample_guass(err.data(), modulus1);

  // the two RNS part share the sample meessage and error
  // intel::hexl::EltwiseFMAMod(temp.data(), message, modulus2, nullptr, length,
  // modulus1, 1); intel::hexl::EltwiseAddMod(temp.data(), temp.data(),
  // err.data(), length, modulus1);
  intel::hexl::EltwiseFMAMod(temp.data(), message, modulus2, err.data(), length,
                             modulus1, 1);
...
void encrypt(uint64_t *b1, uint64_t *a1, uint64_t *b2, uint64_t *a2,
             ...
             uint64_t *secret1, uint64_t *secret2, intel::hexl::NTT &ntts1,
             intel::hexl::NTT &ntts2, uint64_t modulus1, uint64_t modulus2,
             int32_t num = 0) {
  uint64_t modulus = secret.getModulus();
  assert(modulus1 == modulus);
  ...
  intel::hexl::EltwiseFMAMod(err_t.data(), err.data(), (uint64_t)num, nullptr,
                             length, modulus, 1);
  /*******************************************************************/

  // the two RNS part share the sample meessage and error
  intel::hexl::EltwiseAddMod(temp.data(), err_t.data(), message1, length,
                             modulus1);
  ntts1.ComputeForward(temp.data(), temp.data(), 1, 1);
  intel::hexl::EltwiseAddMod(b1, b1, temp.data(), length, modulus1);
  ...
  // TODO: we modify it
  // intel::hexl::EltwiseAddMod(temp.data(), err_t.data(), message2, length,
  // modulus1);
  intel::hexl::EltwiseAddMod(temp.data(), err_t.data(), message2, length,
                             modulus2);
...
void encrypt_rns(RlweCiphertext &cipher1, RlweCiphertext &cipher2,
  ...
  // CRT encryption
  encrypt(cipher1.b.data(), cipher1.a.data(), cipher2.b.data(),
          cipher2.a.data(), secret, message.data(), secret1.data(),
          secret2.data(), // should be ntt form
...
void encrypt_rns(std::vector<uint64_t> &b1, std::vector<uint64_t> &a1,
                 ...
                 std::vector<uint64_t> &message2, int32_t num,
                 uint64_t modulus2) {
  uint64_t length = secret.getLength();
  uint64_t modulus1 = secret.getModulus();
  // uint64_t modulus2 = bigMod2;
  ...
  // CRT encryption
  encrypt(b1.data(), a1.data(), b2.data(), a2.data(), secret, message1.data(),
          message2.data(), secret1.data(), secret2.data(), // should be ntt form
...
// rns encrypt for two messages
#ifdef INTEL_HEXL
void encrypt_bsgs_autokey(uint64_t *b1, uint64_t *a1, uint64_t *b2,
                          uint64_t *a2, Secret &secret, uint64_t *message1,
                          uint64_t *message2, uint64_t *secret1,
                          uint64_t *secret2, intel::hexl::NTT &ntts1,
                          intel::hexl::NTT &ntts2, uint64_t modulus1,
                          uint64_t modulus2) {
  uint64_t modulus = secret.getModulus();
  assert(modulus1 == modulus);
  uint64_t length = secret.getLength();
  ...
  // the two RNS part share the sample meessage and error
  intel::hexl::EltwiseAddMod(temp.data(), err.data(), message1, length,
                             modulus1);
  ntts1.ComputeForward(temp.data(), temp.data(), 1, 1);
  intel::hexl::EltwiseAddMod(b1, b1, temp.data(), length, modulus1);
  ...
  // the second part
  intel::hexl::EltwiseAddMod(temp.data(), err.data(), message2, length,
                             modulus2);
  ntts2.ComputeForward(temp.data(), temp.data(), 1, 1);
  intel::hexl::EltwiseAddMod(b2, b2, temp.data(), length, modulus2);
}
...
void encrypt_rns_bsgs_autokey(std::vector<uint64_t> &b1,
  ...
  **/
  ...
  // CRT encryption
  encrypt_bsgs_autokey(b1.data(), a1.data(), b2.data(), a2.data(), secret,
                       message1.data(), message2.data(), secret1.data(),
                       secret2.data(), // should be ntt form
                       ntts1, ntts2, modulus1, modulus2);
#endif
}
...
void encrypt_special_rlwe(uint64_t *b, uint64_t *a, Secret &secret,
  ...
  intel::hexl::EltwiseFMAMod(err.data(), err.data(), (uint64_t)lwesnum, nullptr,
                             length, modulus, 1);
  /*******************************************************************/
...
void encrypt_rns_bsgs(std::vector<RlweCiphertext> &cipher, Secret &secret,
  ...
  **/
  memcpy(secret1.data(), secret.getData().data(),
         sizeof(uint64_t) * secret.getLength());
  memcpy(secret2.data(), secret.getData().data(),
         sizeof(uint64_t) * secret.getLength());
  ...
  // CRT encryption
  encrypt(cipher[0].b.data(), cipher[0].a.data(), cipher[1].b.data(),
          cipher[1].a.data(), secret, message.data(), secret1.data(),
          secret2.data(), // should be ntt form
...
void decrypt(uint64_t *message, uint64_t *b, uint64_t *a, Secret &secret,
  ...
  intel::hexl::EltwiseMultMod(message, a, secret2.data(), length, modulus2, 1);
