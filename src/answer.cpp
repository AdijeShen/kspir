 ...
 * @param ks
 */
void ksKey_hint2(uint64_t *b_rlwe, uint64_t *a_rlwe, uint64_t *a,
                 uint64_t **ks) {
  uint64_t **dec_a;
  dec_a = new uint64_t *[N];
  for (size_t i = 0; i < N; i++)
...
int ksKey_hint(std::vector<uint64_t> &b_rlwe, std::vector<uint64_t> &a_rlwe,
  ...
  for (size_t j = 0; j < N; j++) {
    for (size_t k = 0; k < ell; k++) {
      ...
      intel::hexl::EltwiseFMAMod(b_rlwe.data(), ks[index].data(), dec_a[k][j],
                                 b_rlwe.data(), N, bigMod, 1);
      intel::hexl::EltwiseFMAMod(a_rlwe.data(), ks[index + 1].data(),
...
int ksKey_hint_first_construction(std::vector<uint64_t> &b_rlwe,
  ...
  for (size_t i = 0; i < N; i++) {
    intel::hexl::EltwiseMultMod(temp[i].data(), a.data(), data[i].data(), N,
                                bigMod, 1);
  ...
  for (size_t j = 0; j < N; j++) {
    ...
    for (size_t k = 0; k < ellnum; k++) {
      ...
      intel::hexl::EltwiseMultMod(t.data(), pk[index].data(), dec_a[k].data(),
                                  N, bigMod, 1);
      intel::hexl::EltwiseAddMod(b_rlwe.data(), b_rlwe.data(), t.data(), N,
                                 bigMod);

      intel::hexl::EltwiseMultMod(t.data(), pk[index + 1].data(),
                                  dec_a[k].data(), N, bigMod, 1);
      intel::hexl::EltwiseAddMod(a_rlwe.data(), a_rlwe.data(), t.data(), N,
                                 bigMod);
    }
  }
...
/*
int ksKey_hint(std::vector<uint64_t>& b_rlwe, std::vector<uint64_t>& a_rlwe,
                std::vector<uint64_t>& a, std::vector<std::vector<uint64_t> >&
ks)
{
    std::vector<std::vector<uint64_t> > dec_a(ell, std::vector<uint64_t>(N, 0));
    ...
    for (size_t j = 0; j < N; j++)
    {
        ...
        {
            int index = 2 * ell * j + 2 * k;
            intel::hexl::EltwiseFMAMod(temp_b.data(), ks[index].data(),
dec_a[k][j], nullptr, N, bigMod, 1); intel::hexl::EltwiseFMAMod(temp_a.data(),
ks[index + 1].data(), dec_a[k][j], nullptr, N, bigMod, 1);

            intel::hexl::EltwiseAddMod(b_rlwe.data(), b_rlwe.data(),
temp_b.data(), N, bigMod); intel::hexl::EltwiseAddMod(b_rlwe.data(),
b_rlwe.data(), temp_a.data(), N, bigMod);
        }
    }
    std::cout << "break 4" << std::endl;
    intel::hexl::EltwiseReduceMod(b_rlwe.data(), b_rlwe.data(), N, bigMod, 1,
1); intel::hexl::EltwiseReduceMod(a_rlwe.data(), a_rlwe.data(), N, bigMod, 1,
1);
...
int answer(uint64_t *b_rlwe, uint64_t *a_rlwe, uint64_t *b, uint64_t *a,
           std::vector<std::vector<uint64_t>> &ks,
           std::vector<std::vector<uint64_t>> &data, bool isntt_b,
           bool isntt_data) {
  ...
  for (size_t i = 0; i < N; i++) {
    ...
    for (size_t j = 0; j < N; j++) {
       ...
       */
      temp += barret_reduce((__int128_t)b[i] *
                            data[i][j]); // the i col of the database
    }
...
int answer_first_dimension(
  ...
  intel::hexl::EltwiseSubMod(a_rlwe.data(), b_vec.data(), a_rlwe.data(), N,
                             bigMod); // -a_rlwe
  ...
  intel::hexl::EltwiseReduceMod(b_vec.data(), b_vec.data(), N, bigMod, bigMod,
                                1);
  intel::hexl::EltwiseFMAMod(b_vec.data(), b_vec.data(), bNinv, nullptr, N,
                             bigMod, 1);

  intel::hexl::EltwiseSubMod(b_rlwe.data(), b_vec.data(), b_rlwe.data(), N,
                             bigMod);
  ...
  // start); std::cout << "hexl answer costs " << glapsed.count() << " us." <<
  // std::endl;
...
int answer_two_dimensions(RlweCiphertext &result, std::vector<uint64_t> &b_rlwe,
                          ...
                          std::vector<std::vector<uint64_t>> &data,
                          RGSWCiphertext &query, bool isntt_b,
                          bool isntt_data) {
  if (!isntt_b | !isntt_data) {
    std::cout << "Error: b or data should be ntt form." << std::endl;
    return 0;
...
void pkKeyGen(std::vector<std::vector<uint64_t>> &pk, Secret &queryKey,
              Secret &answerKey, int32_t ellnum, uint64_t base, uint64_t BBg) {
  ...
  for (size_t i = 0; i < N; i++) {
    ...
    for (size_t j = 0; j < ellnum; j++) {
      intel::hexl::EltwiseFMAMod(temp.data(), mess.data(), mult_factor[j],
                                 nullptr, length, modulus, 1);
      int index = 2 * ellnum * i + 2 * j;

      // TODO: add another encrypt to encrypt cofficient, and this would be
      // faster
...
void dummy_ksKeyGen(std::vector<std::vector<uint64_t>> &ks, Secret &queryKey,
  ...
  intel::hexl::NTT ntts = queryKey.getNTT();
  for (size_t i = 0; i < N; i++) {
    intel::hexl::EltwiseMultMod(mess[i].data(), data[i].data(),
                                queryKey.getData().data(), queryKey.getLength(),
                                queryKey.getModulus(), 1);
  ...
  for (size_t i = 0; i < N; i++) {
    for (size_t j = 0; j < ell; j++) {
      intel::hexl::EltwiseFMAMod(temp.data(), mess[i].data(), mult_factor[j],
                                 nullptr, length, modulus, 1);
...
void evalTrNn(RlweCiphertext &result, int32_t log2Nn, const AutoKey &autokey) {
  ...
  for (size_t k = 1; k <= log2Nn; k++) {
    ...
    // auto glapsed = std::chrono::duration_cast<std::chrono::microseconds>(stop
    // - start); std::cout << "evalAuto costs " << glapsed.count() << " us." <<
    // std::endl;
  }
}
...
/**
 * @brief the variant of packingLWEs algorithm. We packing RLWE ciphertexts
 * instead of only LWE ciphertexts. Refer to Algorithm 2 in
 ...
 * @param lwes the requried packing lwe ciphertexts (note: store them in
 * RlweCiphertext). The size should be power of two. lwes should be store in
 * coefficient form.
 */
void packingLWEs(RlweCiphertext &result, std::vector<RlweCiphertext> &lwes,
  ...
  auto glapsed = std::chrono::duration_cast<std::chrono::microseconds>(stop -
  start); std::cout << "evalTrNn costs " << glapsed.count() << " us." <<
  std::endl;
  */
  result = lwes[0];
   ...
   *  Because we don't have to let unwanted coefficients to be zeros. Therefore,
   *we let evalTrNn function invisible.
   **/
 ...
 *      The size should be power of two. lwes should be store in coefficient
 * form.
 */
...
/**
 * @brief note: in the one-time setup phase, the server should homomorphically
 * evaluate to obtain ks. In other words, is to replace dummy_ksKeyGen function.
 *
 */
...
void evalAutoRNS(RlweCiphertext &result1, RlweCiphertext &result2,
  ...
  std::vector<std::vector<uint64_t>> dec_a2(ellnum,
                                            std::vector<uint64_t>(length, 0));
  decompose_crt(dec_a, dec_a2, temp_a1, temp_a2, ellnum, autokey.getBase(),
                autokey.getBg());
...
void evalExpandRNS(std::vector<RlweCiphertext> &result,
  ...
  for (size_t j = 0; j < l; j++) {
    ...
    for (size_t k = 0; k < (0x01 << j); k++) {
      ...
      addRnsCiphertext(result[second_index], result[second_index + N], temp1,
                       temp2);
    }
  }
  ...
  for (size_t i = 0; i < N; i++) {
    for (size_t j = 0; j < N; j++) {
      ...
      result[i + N].a[j] =
          static_cast<uint128_t>(result[i + N].a[j]) * bNinv2 % bigMod2;
      result[i + N].b[j] =
...
void publicKeyGen(std::vector<std::vector<uint64_t>> &publicKey,
  ...
  for (size_t i = 0; i < N; i++) {
    ...
    for (size_t j = 0; j < ell; j++) {
      intel::hexl::EltwiseFMAMod(temp.data(), message.data(), mult_factor[j],
      ...
      encrypt(publicKey[index].data(), publicKey[index + 1].data(),
              publicKey[index + crt_interval].data(),
              publicKey[index + 1 + crt_interval].data(), answerKey,
              temp.data(), secret1.data(), secret2.data(), // should be ntt form
 ...
 * @param input1 the input rlwe ciphertexts, store in coefficient form. dim:
 * 2*ell
 */
void expand(std::vector<std::vector<uint64_t>> &pk,
            std::vector<RlweCiphertext> &input, AutoKey &autokey) {
...
void evalSingleKsKeyGen(std::vector<std::vector<uint64_t>> &ks,
  ...
  for (size_t j = 0; j < N; j++) {
    ...
    for (size_t k = 0; k < ell; k++) {
      ...
      intel::hexl::EltwiseMultMod(temp.data(), pk[pk_index].data(),
                                  data[data_index].data(), N, modulus1, 1);
      if (!reverse)
        intel::hexl::EltwiseAddMod(ks[output_index].data(),
                                   ks[output_index].data(), temp.data(), N,
                                   modulus1);
      else
        intel::hexl::EltwiseSubMod(ks[output_index].data(),
                                   ks[output_index].data(), temp.data(), N,
                                   modulus1);

      intel::hexl::EltwiseMultMod(temp.data(), pk[pk_index + 1].data(),
                                  data[data_index].data(), N, modulus1, 1);
      if (!reverse)
        intel::hexl::EltwiseAddMod(ks[output_index + 1].data(),
                                   ks[output_index + 1].data(), temp.data(), N,
                                   modulus1);
      else
        intel::hexl::EltwiseSubMod(ks[output_index + 1].data(),
                                   ks[output_index + 1].data(), temp.data(), N,
                                   modulus1);

      // handle CRT 2
      intel::hexl::EltwiseMultMod(temp.data(),
                                  pk[pk_index + crt_interval].data(),
                                  data[data_index + N].data(), N, modulus2, 1);
      if (!reverse)
        intel::hexl::EltwiseAddMod(ks[output_index + crt_interval].data(),
                                   ks[output_index + crt_interval].data(),
                                   temp.data(), N, modulus2);
      else
        intel::hexl::EltwiseSubMod(ks[output_index + crt_interval].data(),
                                   ks[output_index + crt_interval].data(),
                                   temp.data(), N, modulus2);

      intel::hexl::EltwiseMultMod(temp.data(),
                                  pk[pk_index + 1 + crt_interval].data(),
                                  data[data_index + N].data(), N, modulus2, 1);
      if (!reverse)
        intel::hexl::EltwiseAddMod(ks[output_index + 1 + crt_interval].data(),
                                   ks[output_index + 1 + crt_interval].data(),
                                   temp.data(), N, modulus2);
      else
        intel::hexl::EltwiseSubMod(ks[output_index + 1 + crt_interval].data(),
                                   ks[output_index + 1 + crt_interval].data(),
                                   temp.data(), N, modulus2);
    }
  }
...
void modSwitch(RlweCiphertext &result, RlweCiphertext &input1,
  ...
  intel::hexl::EltwiseSubMod(result.a.data(), input1.a.data(), input2.a.data(),
                             length, modulus1);
  intel::hexl::EltwiseFMAMod(result.a.data(), result.a.data(), mod2inv, nullptr,
                             length, modulus1, 1);
  ...
  intel::hexl::EltwiseSubMod(result.b.data(), input1.b.data(), input2.b.data(),
                             length, modulus1);
  intel::hexl::EltwiseFMAMod(result.b.data(), result.b.data(), mod2inv, nullptr,
                             length, modulus1, 1);
