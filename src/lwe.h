...
public:
  ...
  RlweCiphertext(std::vector<uint64_t> &a, std::vector<uint64_t> &b,
                 bool isn = false);
...
public:
  ...
  RGSWCiphertext(uint64_t len, uint64_t module, uint64_t elln, uint64_t base,
                 uint64_t bg);
...
public:
  ...
  AutoKey(int32_t len, uint64_t module, int32_t elln, uint64_t base,
          uint64_t bg);
 ...
 *        the difference from AutoKey is that we have two ellnums, where the one
 * is for baby step and the other one is for giant step
 *
 */
class AutoKeyBSGS {
private:
  ...
  uint64_t PP_bs =
      0; // 0x01 << 8; // 0x01 << 10; // 13;// 0x01 << 11; // the base
...
public:
  ...
  void generateSingleKey(std::vector<RlweCiphertext> &result, int32_t index,
                         Secret &secret, const int32_t ellnum,
                         const uint64_t PP, const uint64_t BBg);

  void keyGen(Secret &secret, const std::vector<int32_t> indexLists,
              StepName stepname = BabyStep);
...
private:
  ...
  uint64_t PP_bs =
      0x01 << 20; // 0x01 << 8; // 0x01 << 10; // 13;// 0x01 << 11; // the base
