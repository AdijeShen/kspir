...
void showLargeIntervalVector(std::vector<uint64_t> &vals, int32_t interval,
  ...
  if (vals.size() <= 16 * interval) {
    for (long i = interval; i < vals.size(); i += interval)
      std::cout << ", " << vals[i];
  } else {
...
uint64_t QInv(int lwenum, uint64_t modulus) {
  ...
  if (modulus == bigMod) {
    ...
    static_assert(bigMod == 1125899906826241,
                  "the function is related to specific modulus.");
    switch (lwenum) {
    case 1:
      result = 1;
      break;
    case 2:
      result = 562949953413121;
      break;
    case 4:
      result = 844424930119681;
      break;
    case 8:
      result = 985162418472961;
      break;
    case 16:
      result = 1055531162649601;
      break;
    case 32:
      result = 1090715534737921;
      break;
    case 64:
      result = 1108307720782081;
      break;
    case 256:
      result = 1121501860315201;
      break;
    case 512:
      result = 1123700883570721;
      break;
    case 4096:
      result = 1125625028919301;
      break;
    default:
      result = 0;
      break;
    ...
    static_assert(bigMod == 18014398509450241,
                  "the function is related to specific modulus.");
    switch (lwenum) {
    case 1:
      result = 1;
      break;
    case 2:
      result = 9007199254725121;
      break;
    case 4:
      result = 13510798882087681;
      break;
    case 8:
      result = 15762598695768961;
      break;
    case 32:
      result = 17451448556029921;
      break;
    case 64:
      result = 17732923532740081;
    default:
      result = 0;
      break;
    }
    ...
    case 1:
      ...
      break;
    case 2:
      result = 33487344869801985;
      break;
    case 4:
      result = 50231017304702977;
      break;
    case 8:
      result = 58602853522153473;
      break;
    case 16:
      result = 62788771630878721;
      break;
    case 32:
      result = 64881730685241345;
      break;
    case 64:
      result = 65928210212422657;
      break;
    case 256:
      result = 66713069857808641;
      break;
    case 512:
      result = 66843879798706305;
      break;
    case 4096:
      result = 66958338496991761;
      break;
    default:
      result = 0;
      break;
...
void multConst(std::vector<uint64_t> &result, uint64_t cosntNum,
               uint64_t modulus) {
#ifdef INTEL_HEXL
  intel::hexl::EltwiseFMAMod(result.data(), result.data(), cosntNum, nullptr, N,
                             modulus, 1);
#endif
...
uint128_t compute_mult_qinvqi(uint64_t input, uint64_t qinv, uint64_t qi) {
  ...
  uint128_t temp =
      static_cast<uint128_t>(input) * input_mult_factor_hignbits; // ac

  uint128_t result =
      (temp >> 64) * static_cast<uint128_t>(two128_mod) % modulus;
...
uint128_t compute_mult_qinvqi_bsgs(uint64_t input, uint64_t qinv, uint64_t qi,
                                   uint64_t modulus2 = bsMod) {
  uint128_t modulus = static_cast<uint128_t>(crtMod) * modulus2;

  uint128_t two128_mod =
      (static_cast<uint128_t>(two128_mod_hignbits_bs) << 63) +
...
uint128_t compute_mult_qinvqi_bsgs_ba(uint64_t input, uint64_t qinv,
                                      uint64_t qi, uint64_t modulus2 = bsMod) {
  uint128_t modulus = static_cast<uint128_t>(crtMod) * modulus2;

  uint128_t two128_mod =
      (static_cast<uint128_t>(two128_mod_hignbits_bsaux) << 63) +
...
uint128_t compute_mult_qinvqi_bsgs_aux(uint64_t input, uint64_t qinv,
                                       uint64_t qi,
                                       uint64_t modulus2 = auxMod) {
  uint128_t modulus = static_cast<uint128_t>(crtMod) * modulus2;

  uint128_t two128_mod =
      (static_cast<uint128_t>(two128_mod_hignbits_aux) << 63) +
 ...
 * input[i] = input1[i] * q2inv * q2 + input2[i] * q1inv * q1 (mod q1*q2)
 */
void crt_inv_bsgs(std::vector<uint128_t> &result,
                  const std::vector<uint64_t> &input1,
                  const std::vector<uint64_t> &input2, uint64_t modulus1,
                  uint64_t modulus2) {
...
void crt_inv_bsgs_aux(std::vector<uint128_t> &result,
                      const std::vector<uint64_t> &input1,
                      const std::vector<uint64_t> &input2, uint64_t modulus1,
                      uint64_t modulus2) {
  assert(modulus1 == crtMod);
...
void crt_inv_bsgs_ba(std::vector<uint128_t> &result,
                     const std::vector<uint64_t> &input1,
                     const std::vector<uint64_t> &input2, uint64_t modulus1,
                     uint64_t modulus2) {
  assert(modulus1 == crtMod);
  ...
  for (size_t i = 0; i < N; i++) {
    result[i] =
        compute_mult_qinvqi_bsgs_ba(input1[i], q2inv, modulus2, modulus2);
    result[i] +=
        compute_mult_qinvqi_bsgs_ba(input2[i], q1inv, modulus1, modulus2);
    result[i] %= modulus;
  }
}
...
void decompose_openfhe(std::vector<std::vector<uint64_t>> &result,
...
void decompose(std::vector<std::vector<uint64_t>> &result,
               const std::vector<uint64_t> &input, int32_t ellnum,
               uint64_t base, uint64_t BBg, uint64_t modulus) {
  // TODO: here use directly macro
  uint64_t length = N;
  // uint64_t modulus = bigMod;
  ...
  // We use unsignd digit decompose firstly and then we substract Bg/2 for each
  // decomposed element. Therefore, adding an offset ensures it always correct
  uint64_t offset = 0;
...
recontruct_bsgs(std::vector<std::vector<uint64_t>> &dec_a1,
                std::vector<std::vector<uint64_t>> &dec_a2, int32_t ellnum,
...
void check_recontruct_bsgs(std::vector<std::vector<uint64_t>> &dec_a1,
                           ...
                           uint64_t base, uint64_t BBg, uint128_t modulus) {
  std::vector<uint128_t> temp =
      recontruct_bsgs(dec_a1, dec_a2, ellnum, base, BBg, modulus);
  ...
  for (size_t i = 0; i < length; i++) {
    ...
    for (size_t j = 0; j < ellnum; j++) {
      ...
      if (r >= 0) {
        ...
        // note: the returned value is in [0, modulus1 - 1] and [0, modulus2 -
        // 1]
...
void compute_hexl_rotate_indexes(std::vector<int32_t> &hexl_ntt_index,
  ...
  for (size_t i = 0; i < log2(length); i++) {
    ...
    for (size_t j = 0; j < current_fill; j++) {
      hexl_ntt_index[j * interval + interval / 2] =
          hexl_ntt_index[j * interval] * (2 * current_fill + 1) %
          (4 * current_fill);
    }
  }
...
inline void compute_inter_permutation(std::vector<int32_t> &inter_permutation,
                                      const std::vector<int32_t> &rotate_index,
                                      int32_t index, int32_t length = N) {
  ...
  for (size_t i = 0; i < length / 2 - index; i++) {
    ...
    // inter_permutation[rotate_index[half_length + i] >> 0x01] =
    // rotate_index[half_length + i + index];
    inter_permutation[rotate_index[length - 1 - i] >> 0x01] =
        rotate_index[length - 1 - i - index];
  }
  for (size_t i = length / 2 - index; i < length / 2; i++) {
    inter_permutation[rotate_index[i] >> 0x01] =
        rotate_index[i + index - length / 2];
    // inter_permutation[rotate_index[half_length + i] >> 0x01] =
    // rotate_index[half_length + i + index - length/2];
    inter_permutation[rotate_index[length - 1 - i] >> 0x01] =
        rotate_index[length - 1 - i - index + length / 2];
  }
