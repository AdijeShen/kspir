...
LweCiphertext::LweCiphertext(int32_t len, uint64_t module)
    : length(len), modulus(module) {
  this->a.resize(len);
}

std::vector<uint64_t> LweCiphertext::getA() { return a; }

uint64_t LweCiphertext::getA(int32_t index) { return a[index]; }

uint64_t LweCiphertext::getB() { return b; }

uint64_t LweCiphertext::getModulus() { return modulus; }

int32_t LweCiphertext::getLength() { return length; }
...
RlweCiphertext::RlweCiphertext(uint64_t len, uint64_t module)
    : length(len), modulus(module) {
  this->a.resize(len);
  this->b.resize(len);
...
RlweCiphertext::RlweCiphertext(std::vector<uint64_t> &a,
                               std::vector<uint64_t> &b, bool isn)
    : isntt(isn) {
  assert(N == a.size());
  assert(N == b.size());
...
/**
 * @brief mult negate of input const. This function is used in the expand
 * function. input ciphertext should be in coefficient form.
 * @param neg_factor
 */
...
std::vector<uint64_t> RlweCiphertext::getA() const { return this->a; }

uint64_t RlweCiphertext::getA(int32_t index) { return this->a[index]; }

std::vector<uint64_t> RlweCiphertext::getB() const { return this->b; }

uint64_t RlweCiphertext::getB(int32_t index) { return this->b[index]; }

uint64_t RlweCiphertext::getModulus() const { return this->modulus; }

void RlweCiphertext::setModulus(uint64_t module) { this->modulus = module; }

int32_t RlweCiphertext::getLength() const { return this->length; }

bool RlweCiphertext::getIsNtt() const { return this->isntt; }

void RlweCiphertext::setIsNtt(bool is) { this->isntt = is; }
...
RGSWCiphertext::RGSWCiphertext(uint64_t len, uint64_t module)
    : length(len), modulus(module) {
  this->a.resize(2 * ellnum);
  this->b.resize(2 * ellnum);
  for (size_t i = 0; i < 2 * ellnum; i++) {
...
RGSWCiphertext::RGSWCiphertext(uint64_t len, uint64_t module, uint64_t elln,
                               uint64_t base, uint64_t bg)
...
void RGSWCiphertext::keyGen(Secret &secret, const uint64_t index,
                            bool is_reverse) {
  ...
  std::vector<uint64_t> mult_factor =
      powerOfBg(this->PP, this->BBg, this->ellnum);
  for (size_t i = 0; i < ellnum; i++) {
#ifdef INTEL_HEXL
    // faster by single barret reduce
    intel::hexl::EltwiseFMAMod(temp.data(), message.data(), mult_factor[i],
                               nullptr, this->length, this->modulus, 1);

    ntts.ComputeForward(temp.data(), temp.data(), 1, 1);

    // rlwe encryption, these rlwes is in ntt form.
    // encrypt(m)
    encrypt(this->b[ellnum + i].data(), this->a[ellnum + i].data(), secret,
            temp.data());

    // encrypt(-ms)
    // intel::hexl::EltwiseSubMod(temp.data(), this->b[ellnum - 1].data(),
    // temp.data(),
    //        this->length, this->modulus);
    negate(temp.data(), this->length, this->modulus);

    intel::hexl::EltwiseMultMod(temp.data(), temp.data(),
                                secret.getData().data(), this->length,
                                this->modulus, 1);
...
*/
uint64_t RGSWCiphertext::getModulus() const { return this->modulus; }

uint64_t RGSWCiphertext::getLength() const { return this->length; }

uint64_t RGSWCiphertext::getEllnum() const { return this->ellnum; }

uint64_t RGSWCiphertext::getBg() const { return this->BBg; }

uint64_t RGSWCiphertext::getBase() const { return this->PP; }

bool RGSWCiphertext::getIsNtt() const { return this->isntt; }

#ifdef INTEL_HEXL
intel::hexl::NTT RGSWCiphertext::getNTT() const { return ntts; }
#endif
...
AutoKey::AutoKey(int32_t len, uint64_t module, int32_t elln, uint64_t base,
                 uint64_t bg)
    : length(len), modulus(module), ellnum(elln), PP(base), BBg(bg) {
...
void AutoKey::generateSingleKey(std::vector<RlweCiphertext> &result,
                                int32_t index, Secret &secret) {
  // The automorphic transform should be done in coefficient form, instead of
  // ntt form. Also, adding both forms in Secret class can save some
  // computation.
  if (secret.isNttForm()) {
    secret.toCoeffForm();
  }
  ...
  for (size_t i = 0; i < ellnum; i++) {
#ifdef INTEL_HEXL
    intel::hexl::EltwiseFMAMod(temp.data(), autosec.data(), mult_factor[i],
                               nullptr, this->length, this->modulus, 1);
    ntts.ComputeForward(temp.data(), temp.data(), 1, 1);
#endif
    // rlwe encryption, these rlwes is in ntt form.
    encrypt(result[i].b.data(), result[i].a.data(), secret, temp.data());
    // encrypt_special_rlwe(result[i].b.data(), result[i].a.data(), secret,
    // temp.data(), num);
...
void AutoKey::generateSingleKey(std::vector<RlweCiphertext> &result,
                                int32_t index, int32_t num, Secret &secret) {
  // The automorphic transform should be done in coefficient form, instead of
  // ntt form. Also, adding both forms in Secret class can save some
  // computation.
  if (secret.isNttForm()) {
...
void AutoKey::keyGen(Secret &secret, const int32_t num, bool packing_rlwe) {
  ...
  for (size_t k = 1; k <= log2Nn; k++) {
     ...
     *  Because we don't have to let unwanted coefficients to be zeros.
     *Therefore, we let evalTrNn function invisible.
     **/
  ...
  for (auto index : indexLists) {
    ...
    keyMap.insert(
        std::pair<int32_t, std::vector<RlweCiphertext>>(index, key_for_index));
  }
}
...
uint64_t AutoKey::getModulus() const { return this->modulus; }

uint64_t AutoKey::getLength() const { return this->length; }

uint64_t AutoKey::getEllnum() const { return this->ellnum; }

uint64_t AutoKey::getBg() const { return this->BBg; }

uint64_t AutoKey::getBase() const { return this->PP; }

bool AutoKey::getIsNtt() const { return this->isntt; }

#ifdef INTEL_HEXL
intel::hexl::NTT AutoKey::getNTT() const { return ntts; }
#endif
...
AutoKeyRNS::AutoKeyRNS(int32_t len, uint64_t module1, uint64_t module2,
                       int32_t elln)
...
// TODO: fix it
void AutoKeyRNS::generateSingleKey(std::vector<RlweCiphertext> &result,
                                   int32_t index, int32_t num, Secret &secret) {
  // The automorphic transform should be done in coefficient form, instead of
  // ntt form. Also, adding both forms in Secret class can save some
  // computation.
  ...
  std::vector<uint128_t> mult_factor =
      powerOfBg128(this->PP, this->BBg, this->ellnum);
  ...
  for (size_t i = 0; i < ellnum; i++) {
    ...
    intel::hexl::EltwiseFMAMod(temp1.data(), autosec.data(), temp, nullptr,
                               this->length, this->modulus1, 1);

    temp = mult_factor[i] % this->modulus2;
    intel::hexl::EltwiseFMAMod(temp2.data(), autosec2.data(), temp, nullptr,
                               this->length, this->modulus2, 1);

    // rns encryption, these rlwes is in ntt form.
    encrypt_rns(result[i].b, result[i].a, result[i + ellnum].b,
                result[i + ellnum].a, secret, temp1, temp2);
    // encrypt_special_rns(result[i].b, result[i].a, result[i + ellnum].b,
    // result[i + ellnum].a,
...
uint64_t AutoKeyRNS::getModulus1() const { return this->modulus1; }

uint64_t AutoKeyRNS::getModulus2() const { return this->modulus2; }

uint64_t AutoKeyRNS::getLength() const { return this->length; }

uint64_t AutoKeyRNS::getEllnum() const { return this->ellnum; }

uint64_t AutoKeyRNS::getBg() const { return this->BBg; }

uint64_t AutoKeyRNS::getBase() const { return this->PP; }

bool AutoKeyRNS::getIsNtt() const { return this->isntt; }

#ifdef INTEL_HEXL
intel::hexl::NTT AutoKeyRNS::getNTT1() const { return ntts1; }

intel::hexl::NTT AutoKeyRNS::getNTT2() const { return ntts2; }
 ...
 */
void externalProduct(RlweCiphertext &result, RlweCiphertext &firstDim,
                     const RGSWCiphertext &query) {
...
#ifdef INTEL_HEXL
  // keyswitch
  std::vector<std::vector<uint64_t>> decFirstDim(
      2 * ellnum, std::vector<uint64_t>(length, 0));
  ...
  // g^-1(a, b) * key
  decompose_rlwe(decFirstDim, firstDim.a, firstDim.b, ellnum, query.getBase(),
                 query.getBg(), modulus);
  for (size_t i = 0; i < 2 * ellnum; i++) {
    ...
    intel::hexl::EltwiseAddMod(result.a.data(), result.a.data(), temp.data(),
                               length, modulus);
  }
  ...
  for (size_t i = 0; i < 2 * ellnum; i++) {
    ...
    intel::hexl::EltwiseAddMod(result.b.data(), result.b.data(), temp.data(),
                               length, modulus);
  }
...
AutoKeyBSGS::AutoKeyBSGS(int32_t len, uint64_t module)
    : length(len), modulus(module) {
#ifdef INTEL_HEXL
  if (module == crtMod) {
    intel::hexl::NTT ntts(N, crtMod, root_of_unity_crt);
...
AutoKeyBSGS::AutoKeyBSGS(int32_t len, uint64_t module, int32_t elln_bs,
                         int32_t elln_gs)
...
void AutoKeyBSGS::generateSingleKey(std::vector<RlweCiphertext> &result,
                                    int32_t index, Secret &secret,
 ...
 * @param num packing numbers
 */
void AutoKeyBSGS::keyGen(Secret &secret, const std::vector<int32_t> indexLists,
                         StepName stepname) {
...
uint64_t AutoKeyBSGS::getModulus() const { return this->modulus; }

uint64_t AutoKeyBSGS::getLength() const { return this->length; }

uint64_t AutoKeyBSGS::getEllnumBS() const { return this->ellnum_bs; }

uint64_t AutoKeyBSGS::getBgBS() const { return this->BBg_bs; }

uint64_t AutoKeyBSGS::getBaseBS() const { return this->PP_bs; }

uint64_t AutoKeyBSGS::getEllnumGS() const { return this->ellnum_gs; }

uint64_t AutoKeyBSGS::getBgGS() const { return this->BBg_gs; }

uint64_t AutoKeyBSGS::getBaseGS() const { return this->PP_gs; }

bool AutoKeyBSGS::getIsNtt() const { return this->isntt; }

#ifdef INTEL_HEXL
intel::hexl::NTT AutoKeyBSGS::getNTT() const { return ntts; }
#endif
...
AutoKeyBSGSRNS::AutoKeyBSGSRNS(int32_t len, uint64_t module, uint64_t bsModulue)
    : length(len), modulus(module), bsModulus(bsModulue) {
#ifdef INTEL_HEXL
  if (module == crtMod) {
    intel::hexl::NTT ntts(N, crtMod, root_of_unity_crt);
    this->ntts = ntts;
...
AutoKeyBSGSRNS::AutoKeyBSGSRNS(int32_t len, uint64_t module, uint64_t bsModule,
    ...
    : length(len), modulus(module), bsModulus(bsModule), ellnum_bs(elln_bs),
      PP_bs(pp_bs), BBg_bs(bbg_bs) {
#ifdef INTEL_HEXL
  if (module == crtMod) {
    intel::hexl::NTT ntts(N, crtMod, root_of_unity_crt);
    this->ntts = ntts;
...
void AutoKeyBSGSRNS::generateSingleKeyBS(std::vector<RlweCiphertext> &result,
                                         int32_t index, Secret &secret,
                                         const int32_t ellnum,
                                         const uint64_t PP,
                                         const uint64_t BBg) {
  // The automorphic transform should be done in coefficient form, instead of
  // ntt form. Also, adding both forms in Secret class can save some
  ...
  for (size_t i = 0; i < ellnum; i++) {
    ...
    intel::hexl::EltwiseFMAMod(temp2.data(), autosec2.data(), temp, nullptr,
                               this->length, this->bsModulus, 1);

    // rns encryption, these rlwes is in ntt form.
    encrypt_rns_bsgs_autokey(result[i].b, result[i].a, result[i + ellnum].b,
                             result[i + ellnum].a, secret, temp1, temp2,
                             bsModulus);
...
void AutoKeyBSGSRNS::generateSingleKeyGS(std::vector<RlweCiphertext> &result,
                                         int32_t index, Secret &secret,
                                         const int32_t ellnum,
                                         const uint64_t PP,
                                         const uint64_t BBg) {
 ...
 * @param num packing numbers
 */
void AutoKeyBSGSRNS::keyGen(Secret &secret,
                            const std::vector<int32_t> indexLists,
                            StepName stepname) {
  // if the input index lists are for gaint step, use the second parameters
  if (stepname == GaintStep) {
    std::vector<RlweCiphertext> key_for_index(this->ellnum_gs,
                                              RlweCiphertext(N, modulus));

    for (auto index : indexLists) {
      // encrypt automorphic^()(s)
      this->generateSingleKeyGS(key_for_index, index, secret, this->ellnum_gs,
                                this->PP_gs, this->BBg_gs);

      keyMap.insert(std::pair<int32_t, std::vector<RlweCiphertext>>(
          index, key_for_index));
    }
  ...
  std::vector<RlweCiphertext> key_for_index(this->ellnum_bs,
                                            RlweCiphertext(N, modulus));
  ...
  for (auto index : indexLists) {
    // encrypt automorphic^()(s)
    this->generateSingleKeyBS(key_for_index, index, secret, this->ellnum_bs,
                              this->PP_bs, this->BBg_bs);
...
uint64_t AutoKeyBSGSRNS::getModulus() const { return this->modulus; }

uint64_t AutoKeyBSGSRNS::getBSModulus() const { return this->bsModulus; }

uint64_t AutoKeyBSGSRNS::getLength() const { return this->length; }

uint64_t AutoKeyBSGSRNS::getEllnumBS() const { return this->ellnum_bs; }

uint64_t AutoKeyBSGSRNS::getBgBS() const { return this->BBg_bs; }

uint64_t AutoKeyBSGSRNS::getBaseBS() const { return this->PP_bs; }

uint64_t AutoKeyBSGSRNS::getEllnumGS() const { return this->ellnum_gs; }

uint64_t AutoKeyBSGSRNS::getBgGS() const { return this->BBg_gs; }

uint64_t AutoKeyBSGSRNS::getBaseGS() const { return this->PP_gs; }

bool AutoKeyBSGSRNS::getIsNtt() const { return this->isntt; }

#ifdef INTEL_HEXL
intel::hexl::NTT AutoKeyBSGSRNS::getNTT() const { return ntts; }

intel::hexl::NTT AutoKeyBSGSRNS::getBSNTT() const { return bsNtts; }
