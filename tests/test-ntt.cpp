...
void test_reduce_7681() {
  ...
  std::cout << "barret reduce time costs " << glapsed.count() << " us"
            << std::endl;
  ...
  stop = std::chrono::high_resolution_clock::now();
  glapsed = std::chrono::duration_cast<std::chrono::microseconds>(stop - start);
  std::cout << "native reduce time costs " << glapsed.count() << " us"
            << std::endl;
...
void test_reduce() {
  ...
  output[0] =
      barret_reduce((uint128_t)input1[0] * input2[0]); // note: error reduce
}
...
{  //  NOLINT
  ...
  //auto input1 =
  //    intel::hexl::GenerateInsecureUniformIntRandomValues(input_size, 0, 100 *
modulus);
