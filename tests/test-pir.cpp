...
void test_pir() {
  ...
  std::cout << "BSGS parameters: (N1: " << N1 << ", N2: " << N2 << ")"
            << std::endl
            << std::endl;
  std::cout << "target_col: " << target_col
            << ", target_packing: " << target_packing << std::endl
            << std::endl;
  ...
  AutoKeyBSGSRNS autoKey(N, crtMod,
                         bsMod); // key-switching keys for BSGS algorithm
  autoKey.bsgsKeyGen(queryKey, N1);

  AutoKey packingKey(length, modulus, 4, 0,
                     0x01 << 14); // key-switching keys for packing algorithm
  ...
  for (size_t i = 0; i < ntimes; i++) {
    matrix_vector_mul_bsgs_rns_crt_large(result, query1, datacrt, autoKey, N1,
                                         permutations,
                                         r); // the first dimension folding
    for (size_t j = 0; j < r; j++)
      externalProduct(ext_rlwes[j], result[j],
                      queryGsw); // the first dimension folding
  ...
  std::cout << " online server response costs " << glapsed.count() << " us."
            << std::endl;
  ...
  // output result
  std::cout << std::endl;
  // std::cout << "the recovered value is " << decryptd_message[row] <<
  // std::endl;
