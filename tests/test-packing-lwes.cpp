...
void test_packing_lwes() {
  ...
  auto glapsed_b =
      std::chrono::duration_cast<std::chrono::microseconds>(stop_b - start_b);
  std::cout << "encryption costs " << glapsed_b.count() << " us." << std::endl;

  AutoKey autokey(length, modulus, 4, 0, 0x01 << 14);
  // AutoKey autokey(length, modulus, 3, 0x01 << 2, 0x01 << 18); section 4.2 in
  // eprint 2020-015 AutoKey autokey;
  ...
  std::cout << ntimes << " packing lwes costs " << glapsed.count() << " us."
            << std::endl;

  // decrypt message
  decrypt(decryptd_message, result, answerKey);

  multConst(decryptd_message, QInv(lwesNum, modulus),
            modulus); // remove term 2^l
  std::cout << std::endl;

  showLargeIntervalVector(decryptd_message, length / lwesNum,
                          "The decrypted value is ");
}

/**
 * @brief packing/convert r RLWEs into a single RLWE
 * algorithms include external product + the subprocedure of r-LWE-to-RLWE
 * algorithm in [https://eprint.iacr.org/2020/015].
 *
...
void test_packing_rlwes() {
  ...
  std::vector<std::vector<uint64_t>> message(rlwesNum,
  ...
  auto glapsed_pk =
      std::chrono::duration_cast<std::chrono::microseconds>(stop_pk - start_pk);
  std::cout << "building packing keys costs " << glapsed_pk.count() << " us."
            << std::endl;
  ...
  auto glapsed =
      std::chrono::duration_cast<std::chrono::microseconds>(stop - start);
  std::cout << ntimes << " packing rlwes costs " << glapsed.count() << " us."
            << std::endl;

  // decrypt message
  decrypt(decryptd_message, result, answerKey);

  multConst(decryptd_message, QInv(rlwesNum, modulus),
            modulus); // remove term 2^l
  std::cout << std::endl;
