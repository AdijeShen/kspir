...
void generate_offline_key(AutoKeyBSGSRNS &offlineKey1,
                          AutoKeyBSGSRNS &offlineKey2, AutoKeyBSGSRNS &autoKey1,
                          AutoKeyBSGSRNS &autoKey2, Secret &queryKey,
                          int32_t N1) {
  std::vector<int32_t> indexLists;
...
void test_generate_autokey() {
  ...
  AutoKeyBSGSRNS autoKey1(N, crtMod, crtBaMod, 3, (0x01 << 20) * auxMod,
                          0x01 << 20);
  AutoKeyBSGSRNS autoKey2(N, crtMod, auxMod, 3, (0x01 << 11) * auxMod,
                          0x01 << 15);
  generate_offline_key(offlineKey1, offlineKey2, autoKey1, autoKey2, queryKey,
                       N1);
  ...
  for (size_t i = 0; i < ntimes; i++) {
    genAutoKeyFromOffline(result, autoKey1, autoKey2, offlineKey1, offlineKey2,
                          N1);
  }
  auto stop = std::chrono::high_resolution_clock::now();
  ...
  auto glapsed =
      std::chrono::duration_cast<std::chrono::microseconds>(stop - start);
  std::cout << ntimes << " generate auto keys costs " << glapsed.count()
            << " us." << std::endl;
}
