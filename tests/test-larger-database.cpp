...
void database_init(std::vector<std::vector<uint64_t>> &data, uint64_t row = 123,
                   uint64_t col = 123) {
...
void test_larger_database() {
  ...
  database_init(data, row, col)
      ...
      build_query() RlweCiphertext query1(N, bigMod);
  ...
  auto glapsed_ksk = std::chrono::duration_cast<std::chrono::milliseconds>(
      stop_ksk - start_ksk);
  std::cout << " ksk hint time costs " << glapsed_ksk.count() << " ms"
            << std::endl;
  ...
  auto glapsed_ans = std::chrono::duration_cast<std::chrono::milliseconds>(
      stop_ans - start_ans);
  std::cout << " answer time costs " << glapsed_ans.count() << " ms"
            << std::endl;
