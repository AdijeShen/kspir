...
void test_setup() {
  ...
  std::vector<std::vector<uint64_t>> setup_data(2 * N,
  ...
  database_tontt(data);
  std::vector<std::vector<uint64_t>> ks2(2 * ell * N,
                                         std::vector<uint64_t>(N, 0));
  dummy_ksKeyGen(ks2, queryKey, answerKey, data);
  /******************************************************************/

  std::vector<std::vector<uint64_t>> pk(2 * ell * N * 2,
                                        std::vector<uint64_t>(N, 0));
  publicKeyGen(pk, queryKey, answerKey);

  // build keyswitch keys
  std::vector<std::vector<uint64_t>> ks(2 * ell * N * 2,
                                        std::vector<uint64_t>(N, 0));
  ...
  std::vector<std::vector<uint64_t>> result(2 * ell * N,
                                            std::vector<uint64_t>(N, 0));
  modSwitch(result, ks);
  // modSwitch(result, pk);
  ...
  std::cout << " evaluate kskeygen time costs " << glapsed.count() << " ms"
            << std::endl;
  ...
  decrypt(decrypted_crt2.data(), pk[0 + crt_interval].data(), pk[1 +
  crt_interval].data(), answerKey, bigMod2); showLargeVector(decrypted_crt2,
  "crt2 is ");
  */
}
