...
void reorientCipher(uint64_t *cipherbuf,
                    std::vector<std::vector<uint64_t>> &inputAcrt,
                    std::vector<std::vector<uint64_t>> &inputBcrt, int32_t N1,
                    int32_t ringdim) {
  ...
  for (size_t i = 0; i < ringdim; i++) {
    for (size_t j = 0; j < N1; j++) {
      ...
      // cipherbuf[i * 2 * N1 + 2 * j] = barrett_coeff(temp, 0) |
      // barrett_coeff(temp, 1) << 32; temp = inputB[j][i]; cipherbuf[i * 2 * N1
      // + 2 * j + 1] = barrett_coeff(temp, 0) | barrett_coeff(temp, 1) << 32;
      cipherbuf[i * 2 * N1 + 2 * j] =
          inputAcrt[j][i] | inputAcrt[j][i + ringdim] << 32;
      cipherbuf[i * 2 * N1 + 2 * j + 1] =
          inputBcrt[j][i] | inputBcrt[j][i + ringdim] << 32;
    }
  }
}
...
void database_tocrt(uint64_t *datacrt,
                    std::vector<std::vector<uint64_t>> &data_ntt,
                    const int32_t firstdim, const int32_t seconddim,
                    const int32_t ringdim) {
  ...
  for (size_t i = 0; i < ringdim; i++) {
    for (size_t j = 0; j < seconddim; j++) {
      for (size_t k = 0; k < firstdim; k++) {
        ...
        // TODO: check
        // datacrt[i * firstdim * seconddim + j * firstdim + k] =
        // barrett_coeff(temp, 0) | barrett_coeff(temp, 1) << 32;
        datacrt[i * firstdim * seconddim + j * firstdim + k] = temp | temp2
                                                                          << 32;
      }
    }
  }
...
void test_mul256MB() {
  ...
  std::vector<std::vector<uint64_t>> data(total_vec,
                                          std::vector<uint64_t>(ringdim, 0));
  std::vector<std::vector<uint64_t>> data_ntt(
      total_vec, std::vector<uint64_t>(ringdim, 0));
  // sample database
  ...
  for (size_t i = 0; i < total_vec; i++) {
    ntts.ComputeForward(data_ntt[i].data(), data[i].data(), 1,
                        1); // convert to ntt form
  }

  // dummy ciphertext
  std::vector<std::vector<uint64_t>> inputA(firstdim,
                                            std::vector<uint64_t>(ringdim, 0));
  std::vector<std::vector<uint64_t>> inputB(firstdim,
                                            std::vector<uint64_t>(ringdim, 0));
  ...
  std::vector<std::vector<uint64_t>> resultA(seconddim,
                                             std::vector<uint64_t>(ringdim, 0));
  std::vector<std::vector<uint64_t>> resultB(seconddim,
                                             std::vector<uint64_t>(ringdim, 0));
  ...
  for (size_t nt = 0; nt < ntimes; nt++) {
    for (size_t i = 0; i < seconddim; i++) {
      for (size_t j = 0; j < firstdim; j++) {
        intel::hexl::EltwiseMultMod(tempA.data(), inputA[j].data(),
                                    data_ntt[i * firstdim + j].data(), ringdim,
                                    modulus, 1);
        intel::hexl::EltwiseMultMod(tempB.data(), inputB[j].data(),
                                    data_ntt[i * firstdim + j].data(), ringdim,
                                    modulus, 1);
        intel::hexl::EltwiseAddMod(resultA[i].data(), resultA[i].data(),
                                   tempA.data(), ringdim, modulus);
        intel::hexl::EltwiseAddMod(resultB[i].data(), resultB[i].data(),
                                   tempB.data(), ringdim, modulus);
      }
    }
  }
  ...
  std::cout << "dummy multiplication costs " << glapsed.count() << " us."
            << std::endl;
  ...
  // crt version
  std::vector<std::vector<uint64_t>> inputAcrt(
      firstdim, std::vector<uint64_t>(2 * ringdim, 0));
  std::vector<std::vector<uint64_t>> inputBcrt(
      firstdim, std::vector<uint64_t>(2 * ringdim, 0));
  for (size_t i = 0; i < firstdim; i++) {
    computeForward(inputAcrt[i].data(), inputA[i].data());
    computeForward(inputBcrt[i].data(), inputB[i].data());
  }
  std::vector<std::vector<uint64_t>> data_nttcrt(
      total_vec, std::vector<uint64_t>(2 * ringdim, 0));
  for (size_t i = 0; i < total_vec; i++) {
    computeForward(data_nttcrt[i].data(), data[i].data());
  }

  uint64_t *cipherbuf =
      (uint64_t *)aligned_alloc(64, sizeof(uint64_t) * firstdim * 2 * N);
  ...
  std::vector<std::vector<uint64_t>> output(seconddim,
                                            std::vector<uint64_t>(4 * N, 0));

  auto start_crt = std::chrono::high_resolution_clock::now();
  for (size_t i = 0; i < ntimes; i++) {
    fastMultiplyQueryByDatabaseDim1(output, datacrt, cipherbuf, firstdim,
                                    seconddim);
  }
  ...
  auto glapsed_crt = std::chrono::duration_cast<std::chrono::microseconds>(
      stop_crt - start_crt);
  std::cout << "crt dummy multiplication costs " << glapsed_crt.count()
            << " us." << std::endl;
