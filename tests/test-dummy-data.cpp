...
void test_dummy_data() {
  ...
  std::cout << "database initialization time costs " << glapsed.count()
            << " us." << std::endl;
...
#ifndef INTEL_HEXL
    // answer(result_b.data(), result_a.data(), query_b.data(), query_a.data(),
    // data, ks);
#else
  ...
  std::cout << ntimes << " time costs " << glapsed.count() << " us."
            << std::endl;
}
